...
let polygon (sides:int) length plotter =
    ...
    Seq.fold (fun s _ -> turn angle (move length s)) plotter [1.0..(float sides)]

let semiCircle (sides:int) length plotter =
    let angle = Math.Round(360.0/float sides)
    Seq.fold (fun s _ -> turn angle (move length s)) plotter [1.0..(float sides/2.0)]

let thirdCircle (sides:int) length plotter =
    let angle = Math.Round(360.0/float sides)
    Seq.fold (fun s _ -> turn angle (move length s)) plotter [1.0..(float sides/3.0)]

let fifthCircle (sides:int) length plotter =
    let angle = Math.Round(360.0/float sides)
    Seq.fold (fun s _ -> turn angle (move length s)) plotter [1.0..(float sides/5.0)]

let fifteenthCircle (sides:int) length plotter =
    let angle = Math.Round(360.0/float sides)
    Seq.fold (fun s _ -> turn angle (move length s)) plotter [1.0..(float sides/15.0)]
...
let generate cmdStripe times fromPlotter =
    ...
    cmdsGen 
    |> Seq.take times
    |> Seq.fold (fun plot cmd -> cmd plot) fromPlotter
    *)
